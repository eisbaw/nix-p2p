...
E2E_FAST := "--only s1-byte-and-counts --only narinfo-default-cache-offload --only s2-fallback --only tamper-narhash --only chain-s1-and-counts --only s6-p2p --only s9-libp2p-grow --only libp2p-leech --only libp2p-bootstrap-outage --only libp2p-mdns-bootstrap --only libp2p-mdns-scope-isolation"
