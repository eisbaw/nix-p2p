...
impl Endpoint {
    ...
    ///
    /// On native targets, when this future returns, the endpoint's QUIC runtime
    /// has shut down and every Iroh-managed IP-transport UDP socket has
    /// completed its underlying OS close. This native release guarantee holds
    /// even while clones of the respective [`Endpoint`] remain alive, so the
    /// same fixed addresses can be rebound immediately after awaiting this
    /// method.
