...
pub(crate) struct EndpointInner {
    ...
    // Runtime used by noq
    runtime: Arc<Runtime>,
    /// Stable handles to every bound IP socket. The Noq transport and its
    /// senders also hold clones, so dropping the transport cannot serve as a
    /// synchronous socket-release barrier.
    #[cfg(not(wasm_browser))]
...
struct ShutdownState {
    ...
    ///
    /// The socket actor observes this token; after QUIC drain it is explicitly
    /// aborted and joined before the release barrier can complete.
    at_endpoint_closed: CancellationToken,
    /// Elects the single task that drives the release barrier.
    close_started: AtomicBool,
    /// Wakes every [`crate::Endpoint::close`] caller once the release barrier
    /// has either completed or failed.
    close_complete: CancellationToken,
    /// Set before [`Self::close_complete`] is cancelled if the release barrier
    /// did not complete successfully.
    close_failed: AtomicBool,
    /// Set if the native endpoint's QUIC runtime and retained IP-transport UDP
    /// sockets have completed their release barrier.
    ///
    /// On native targets this is only set once the actor has been joined, the
    /// QUIC runtime has shut down, and every retained IP-transport UDP socket
    /// has completed its OS close.
    closed: AtomicBool,
}
...
impl ShutdownState {
    /// Whether the endpoint resource release barrier completed.
    ///
    /// An independent actor failure may still make close callers fail.
    fn is_closed(&self) -> bool {
        self.closed.load(Ordering::Acquire)
    }
}

/// Ensures a dropped or panicking close driver never strands its waiters.
///
/// A successful driver disarms this guard only after the complete resource
/// release barrier. Every other exit publishes failure before waking waiters.
...
impl CloseCompletionGuard {
    fn new(sock: Arc<Socket>) -> Self {
        Self { sock, armed: true }
    }

    /// Publishes successful completion of the release barrier.
    fn complete(mut self) {
        self.sock.shutdown.closed.store(true, Ordering::Release);
        self.sock.shutdown.close_complete.cancel();
        self.armed = false;
    }

    /// Publishes a close failure after resource release still completed.
    fn fail_after_release(mut self) {
        self.sock.shutdown.closed.store(true, Ordering::Release);
        self.sock
            .shutdown
            .close_failed
...
/// Abort the socket actor and prove its future (and owned resources) dropped.
async fn abort_and_join_actor(actor_task: AbortOnDropHandle<()>) -> Result<(), task::JoinError> {
    actor_task.abort();
...
impl EndpointInner {
    ...
    ///
    /// The first caller starts a detached close driver. Every caller waits for
    /// that same driver, so cancelling one caller cannot cancel the resource
    /// release barrier. Polling the socket ([`noq::AsyncUdpSocket::poll_recv`])
    /// will return [`Poll::Pending`] indefinitely after shutdown starts.
    ///
    ...
    pub(crate) async fn close(self: &Arc<Self>) {
        if self
            ...
            .is_ok()
        {
            // This exact guard moves into the driver future. If spawning
            // panics, or the spawned future is cancelled before its first poll,
            // dropping the future still publishes terminal failure.
            let completion = CloseCompletionGuard::new(self.sock.clone());

            // Cancel this synchronously with the first poll, before the
            // detached driver can be scheduled, so shutdown observers see a
            // single well-defined transition.
            self.sock.shutdown.at_close_start.cancel();

            // Initiate the endpoint close before spawning: callers that poll
            // close once use this as an observed endpoint-first ordering
            // boundary.
            self.sock.address_lookup().clear();
            self.noq_endpoint().close(0u16.into(), b"");
            let inner = self.clone();
            let driver = task::spawn(async move {
                inner.close_driver(completion).await;
            });
            drop(driver);
        }

        self.sock.shutdown.close_complete.cancelled().await;
        if self.sock.shutdown.close_failed.load(Ordering::Acquire) {
            if self.sock.shutdown.closed.load(Ordering::Acquire) {
                panic!(
                    "iroh endpoint close reported an actor failure after the endpoint resource-release barrier completed"
                );
            }
            panic!(
                "iroh endpoint close driver failed before the endpoint resource-release barrier completed"
            );
        }
    }

    async fn close_driver(self: Arc<Self>, completion: CloseCompletionGuard) {
        ...
        // MutexGuard is not held across await points. QUIC draining is already
        // complete, so abort the socket actor and then join that same handle.
        // Cancellation alone is insufficient: the actor can be blocked inside
        // an awaited mailbox send and never return to its outer cancellation
        // select. Dropping an aborted handle would also be insufficient because
        // it would not prove that the future and its socket clones were dropped.
        let task = self
            .actor_task
            .lock()
            .unwrap_or_else(|poisoned| {
                warn!("recovering poisoned actor-task mutex during endpoint close");
                poisoned.into_inner()
            })
            .take();
        let mut actor_failure = None;
        if let Some(task) = task
            && let Err(err) = abort_and_join_actor(task).await
        {
            warn!("socket actor failed while completing endpoint close: {err}");
            actor_failure = Some(err);
        }
        ...
        // `netwatch::UdpSocket::drop` queues the potentially blocking libc
        // close in an unawaited `spawn_blocking` task. Explicit close provides
        // the release barrier promised by `Endpoint::close`, even while other
        // Endpoint or sender clones remain alive.
        #[cfg(not(wasm_browser))]
        for socket in &self.ip_sockets {
            socket.close().await;
        }

        trace!("socket closed");
        if actor_failure.is_some() {
            completion.fail_after_release();
        } else {
            completion.complete();
        }
    }
    ...
    /// - Publishes terminal close failure to any theoretical waiter
    ///
    ...
    pub(crate) fn abort(&self) {
        ...
        // Publish the start transition even when shutdown originates from
        // Drop instead of Endpoint::close.
        self.sock
            .shutdown
            .close_started
            .store(true, Ordering::Release);
        ...
        self.actor_task
            .lock()
            .unwrap_or_else(|poisoned| {
                warn!("recovering poisoned actor-task mutex during endpoint abort");
                poisoned.into_inner()
            })
            .take();

        // Abort is deliberately not a successful release barrier: the actor
        // and runtime are cancelled without joins, and UDP close completion is
        // not awaited. Wake any theoretical waiter, but make it fail fast.
        self.sock
            .shutdown
            .close_failed
            .store(true, Ordering::Release);
...
mod tests {
    ...
    use super::{CloseCompletionGuard, Options, abort_and_join_actor};
    use crate::{
        Endpoint, SecretKey,
    ...
    async fn poll_once_is_pending<F: Future>(mut future: Pin<&mut F>) -> bool {
        std::future::poll_fn(|cx| Poll::Ready(future.as_mut().poll(cx).is_pending())).await
    }

    #[test]
    fn fixed_port_release_actor_abort_join_waits_for_future_drop() {
        ...
        runtime.block_on(async {
            ...
            let actor = tokio::spawn(PendingUntilDropped {
                dropped: dropped.clone(),
            });
            tokio::time::timeout(
                Duration::from_secs(2),
                abort_and_join_actor(AbortOnDropHandle::new(actor)),
            )
            .await
            .expect("actor abort-and-join must not hang")
            .expect("cancelled actor is a successful teardown");
            assert!(
                dropped.load(std::sync::atomic::Ordering::Acquire),
                "actor teardown must prove its future was dropped before returning"
            );
        });
    }

    async fn exercise_fixed_port_close_barrier(
        cancel_first_caller: bool,
        install_stuck_actor: bool,
    ) {
        let reserved = std::net::UdpSocket::bind((std::net::Ipv4Addr::LOCALHOST, 0))
            .expect("reserve an ephemeral UDP port");
        let fixed_addr = reserved.local_addr().expect("reserved local address");
        drop(reserved);
        ...
        let endpoint = Endpoint::builder(presets::Minimal)
            ...
            .bind_addr(fixed_addr)
            .expect("valid fixed address")
            .bind()
            .await
            .expect("bind first endpoint");
        let inner = endpoint.inner().expect("open endpoint inner");
        assert_eq!(inner.ip_sockets.len(), 1, "test owns one fixed socket");
        let retained_socket = inner.ip_sockets[0].clone();
        assert_eq!(
            endpoint
                .address_lookup()
                .expect("open endpoint address lookup")
                .len(),
            1,
            "test endpoint starts with one lookup service"
        );

        if install_stuck_actor {
            let real_actor = inner
                .actor_task
                .lock()
                .expect("actor-task mutex")
                .take()
                .expect("bound endpoint actor");
            tokio::time::timeout(Duration::from_secs(2), abort_and_join_actor(real_actor))
                .await
                .expect("real actor teardown must not hang")
                .expect("real actor teardown");

            // This synthetic actor never observes at_endpoint_closed. It also
            // retains a socket handle, matching the resource ownership that a
            // real actor can have while stuck inside an awaited mailbox send.
            let actor_socket = retained_socket.clone();
            let stuck_actor = tokio::spawn(async move {
                let _actor_socket = actor_socket;
            ...
            let replaced = inner
                ...
                .lock()
                .expect("actor-task mutex")
                .replace(AbortOnDropHandle::new(stuck_actor));
            assert!(
                replaced.is_none(),
                "real actor was removed before replacement"
            );
        }
        ...
        blocking_started_rx
            .await
            .expect("sole blocking worker started");

        let endpoint_to_close = endpoint.clone();
        let mut first_close = Some(Box::pin(endpoint_to_close.close()));
        assert!(
            poll_once_is_pending(first_close.as_mut().expect("first close").as_mut()).await,
            "the first close poll starts, but cannot complete, the release barrier"
        );
        assert!(
            endpoint
                .address_lookup()
                .expect("open endpoint address lookup")
                .is_empty(),
            "the first close poll must synchronously start endpoint shutdown"
        );
        tokio::time::timeout(Duration::from_secs(2), async {
            while !retained_socket.is_closed() {
                tokio::task::yield_now().await;
            }
        })
        .await
        .expect("endpoint reached the explicit UDP-close barrier");

        if cancel_first_caller {
            drop(first_close.take());
        }

        // A second caller must join the internally-owned driver. Poll exactly
        // once at the observed netwatch barrier: a Ready result here proves a
        // false-success race without relying on timing or sleeps.
        let mut second_close = Box::pin(endpoint.close());
        assert!(
            poll_once_is_pending(second_close.as_mut()).await,
            "a concurrent close must wait for the queued libc close"
        );
        if let Some(first_close) = first_close.as_mut() {
            assert!(
                poll_once_is_pending(first_close.as_mut()).await,
                "the first close must wait for the queued libc close"
            );
        }

        release_blocking_tx
            .send(())
            .expect("release sole blocking worker");
        tokio::time::timeout(Duration::from_secs(2), async {
            second_close.await;
            if let Some(first_close) = first_close {
                first_close.await;
            }
            blocking_worker.await.expect("blocking worker task");
            ...
            let restarted = Endpoint::builder(presets::Minimal)
                ...
                .clear_address_lookup()
                .bind_addr(fixed_addr)
                .expect("valid restart address")
                .bind()
                .await
                .expect("awaited close must permit immediate fixed-port restart");
            restarted.close().await;
        })
        .await
        .expect("release barrier and immediate fixed-port restart must complete");
    }

    #[test]
    fn concurrent_endpoint_close_waits_for_udp_close_before_fixed_port_rebind() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .max_blocking_threads(1)
            .build()
            .expect("test runtime");
        runtime.block_on(exercise_fixed_port_close_barrier(false, false));
    }

    #[test]
    fn cancelled_endpoint_close_caller_does_not_cancel_fixed_port_release() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .max_blocking_threads(1)
            .build()
            .expect("test runtime");
        runtime.block_on(exercise_fixed_port_close_barrier(true, false));
    }

    #[test]
    fn stuck_endpoint_actor_cannot_strand_fixed_port_release() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .max_blocking_threads(1)
            .build()
            .expect("test runtime");
        runtime.block_on(exercise_fixed_port_close_barrier(false, true));
    }

    #[test]
    fn unpolled_close_driver_publishes_fixed_port_release_failure() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .expect("test runtime");
        runtime.block_on(async {
            ...
            let inner = endpoint.inner().expect("open endpoint inner").clone();
            inner
                .sock
                .shutdown
                .close_started
            ...
            let driver = inner
                .clone()
                .close_driver(CloseCompletionGuard::new(inner.sock.clone()));
            drop(driver);

            assert!(
                inner.sock.shutdown.close_failed.load(Ordering::Acquire),
                "dropping an unpolled driver future must publish failure"
            );
            assert!(
                inner.sock.shutdown.close_complete.is_cancelled(),
                "dropping an unpolled driver future must wake close waiters"
            );
            assert!(
                !inner.sock.shutdown.closed.load(Ordering::Acquire),
                "an unpolled driver did not complete the release barrier"
            );
        });
    }

    #[test]
    fn panicked_endpoint_actor_fails_only_after_fixed_port_release() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .expect("test runtime");
        runtime.block_on(async {
            ...
            let panicked_actor = tokio::spawn(async {
                panic!("synthetic socket actor panic");
            });
            while !panicked_actor.is_finished() {
                tokio::task::yield_now().await;
            }
            let replaced = inner
                .actor_task
                .lock()
                .expect("actor-task mutex")
                .replace(AbortOnDropHandle::new(panicked_actor));
            assert!(
                replaced.is_none(),
                "real actor was removed before replacement"
            );
            ...
            let close_waiter = tokio::spawn(async move {
                endpoint_to_close.close().await;
            });
            let close_error = tokio::time::timeout(Duration::from_secs(2), close_waiter)
                .await
                .expect("failed close waiter must not hang")
                .expect_err("actor failure must fail the public close waiter");
            assert!(close_error.is_panic(), "close waiter must fail fast");
            let panic = close_error.into_panic();
            let message = panic
                .downcast_ref::<&str>()
                .copied()
                .or_else(|| panic.downcast_ref::<String>().map(String::as_str))
                .expect("close panic message");
            assert!(
                message.contains("after the endpoint resource-release barrier completed"),
                "unexpected close panic: {message}"
            );
            assert!(
                endpoint.is_closed(),
                "resource release remains complete despite actor failure"
            );
            ...
            let restarted = Endpoint::builder(presets::Minimal)
                ...
                .bind()
                .await
                .expect("actor failure must be published only after fixed-port release");
            tokio::time::timeout(Duration::from_secs(2), restarted.close())
                .await
                .expect("restarted endpoint close must not hang");
        });
    }
