...
pub struct Rpc {
    ...
    /// LOCAL VENDOR PATCH (nix-p2p TASK-284): when `true`, adaptive server-mode
    /// promotion is disabled — see `try_switching_to_server_mode` and
    /// [`Config::no_adaptive`](config::Config::no_adaptive).
    no_adaptive: bool,
}
...
impl Rpc {
    ...
    pub fn new(config: config::Config) -> Result<Self, std::io::Error> {
        ...
        Ok(Rpc {
            ...
            // LOCAL VENDOR PATCH (nix-p2p TASK-284): carry the client-only flag from config.
    ...
    ///
    /// LOCAL VENDOR PATCH (nix-p2p TASK-284): when `no_adaptive` is set (via
    /// [`DhtBuilder::no_adaptive`](crate::DhtBuilder::no_adaptive)), the adaptive
    /// promotion is a NO-OP: a node that did not explicitly request `server_mode`
    /// is NEVER promoted to a serving DHT node, however long it runs and however
    /// reachable it proves to be. This is the strict client-only guarantee nix-p2p's
    /// Mainline peer-address rendezvous depends on. The explicit `server_mode` path is
    /// untouched: a node built with `server_mode()` still serves as before.
    fn try_switching_to_server_mode(&mut self) {
        if self.no_adaptive {
            // Client-only: refuse adaptive promotion regardless of reachability.
            return;
        }
...
mod tests {
    ...
    /// LOCAL VENDOR PATCH ORACLE (nix-p2p TASK-284, mped M2).
    ///
    /// Proves the client-only guarantee at its exact boundary: the ADAPTIVE promotion
    /// step `try_switching_to_server_mode` is inert for a `no_adaptive` node even when
    /// the node is NOT firewalled (the exact condition that promotes stock mainline).
    ///
    /// The companion test `stock_adaptive_promotes_when_not_firewalled` is the POSITIVE
    /// control: it drives the identical condition on a stock (`no_adaptive == false`)
    /// node and shows the promotion DOES fire. Together they are mutation-provable —
    /// reverting the `if self.no_adaptive { return; }` guard in
    /// `try_switching_to_server_mode` turns THIS test RED (the node promotes), while the
    /// control stays GREEN. The two share one construction path, so the only difference
    /// exercised is the flag, and the only thing that can suppress promotion is the patch.
    #[test]
    fn no_adaptive_client_never_promotes_when_not_firewalled() {
        let mut rpc = Rpc::new(config::Config {
            bootstrap: Some(vec![]),
            bind_address: Some(Ipv4Addr::LOCALHOST),
            no_adaptive: true,
        ...
        // Preconditions: a fresh non-server node starts firewalled and client-mode.
        assert!(!rpc.server_mode(), "fresh no_adaptive node must start as a client");
        // Force the reachability precondition that WOULD promote a stock node.
        rpc.firewalled = false;

        // The maintenance step that adaptively promotes. With the patch it is a no-op.
        rpc.try_switching_to_server_mode();

        assert!(
            !rpc.server_mode(),
            "a no_adaptive (client-only) node must NEVER be adaptively promoted to server \
             mode, even when not firewalled (TASK-284 AC#5)"
        );
    }

    /// POSITIVE CONTROL for `no_adaptive_client_never_promotes_when_not_firewalled`.
    /// A stock (`no_adaptive == false`) node under the IDENTICAL not-firewalled condition
    /// IS adaptively promoted — this is the stock mainline behaviour the security fix
    /// suppresses, and it proves the guard (not some unrelated precondition) is what keeps
    /// the client-only node from promoting.
    #[test]
    fn stock_adaptive_promotes_when_not_firewalled() {
        let mut rpc = Rpc::new(config::Config {
            bootstrap: Some(vec![]),
            bind_address: Some(Ipv4Addr::LOCALHOST),
            no_adaptive: false,
        ...
        assert!(!rpc.server_mode(), "fresh node starts as a client");
        rpc.firewalled = false;
        ...
        assert!(
            rpc.server_mode(),
            "sanity: stock adaptive mode DOES promote a non-firewalled node — if this fails, \
             the promotion trigger changed and the client-only oracle above is vacuous"
        );
    }
}
