...
impl DhtBuilder {
    ...
    /// LOCAL VENDOR PATCH (nix-p2p TASK-284): disable adaptive server-mode promotion.
    ///
    /// A node built with this set is NEVER adaptively promoted to a serving DHT node by
    /// the internal `try_switching_to_server_mode` maintenance step — it stays a client
    /// forever unless [`server_mode`](Self::server_mode) was ALSO requested explicitly.
    /// This is the client-only guarantee nix-p2p's Mainline rendezvous relies on (it uses
    /// this crate purely to look up peer addresses and must never serve the public
    /// BitTorrent DHT). See [`Config::no_adaptive`](crate::rpc::config::Config::no_adaptive).
    pub fn no_adaptive(&mut self) -> &mut Self {
        self.0.no_adaptive = true;
