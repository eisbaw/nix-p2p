...
impl Control {
    ...
    pub async fn open_stream(
        ...
        Self::request_stream(&mut new_stream_sender, protocol).await
    }

    /// Open a stream on one exact, already-established connection.
    ///
    /// The `(connection, peer)` association and that connection's sender are resolved while
    /// holding one [`Shared`] lock. A stale ID, an ID belonging to another peer, or a connection
    /// whose handler sender is gone returns [`io::ErrorKind::NotConnected`]. This method never
    /// chooses another connection, initiates a dial, or falls back to [`Self::open_stream`].
    pub async fn open_stream_on_connection(
        ...
        tracing::debug!(%peer, %connection, "Requesting new stream on exact connection");
    ...
    async fn request_stream(
        new_stream_sender: &mut mpsc::Sender<NewStream>,
        protocol: StreamProtocol,
    ) -> Result<Stream, OpenStreamError> {
