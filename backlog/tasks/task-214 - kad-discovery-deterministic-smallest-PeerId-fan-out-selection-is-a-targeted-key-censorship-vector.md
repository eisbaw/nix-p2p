...
- [x] #2 A key whose legit provider is out-competed on one query can still be resolved on retry (self-heals; retry does not re-chase the identical dead subset)
- [x] #3 If the residual risk is accepted rather than fully closed, it is documented in the fan-out threat-model comment with the honest bound
<!-- AC:END -->
...
CODEX RE-GATE (commits 55f39c2 + bd89c1f + comment-fix 17ff413): GO. All 5 items PASS. AC#2 PASS - GREEN arm now draws INDEPENDENT fixed-seed StdRng next_u64 salts through the production retain_bounded_provider fold (retained_under_salt), RED arm valid, mutation-proof intact (drop ^salt collapses to salt-0 -> reddens), ~4e-19 labeled a model estimate. AC#3 PASS - prose now exact=N_v/2^64, idealized-uniform min(1,max_peers/T) approximation, min(1,.) clamp in the exclusion formula, almost-sure (not finite-guaranteed) healing. Regression AC#1/#4/#5 PASS (production per-query salt not key-derived; cap<=max_peers O(max_peers) kad-exclusive presentation-order-salt-independent; integer-only no-float, no frozen surface). One non-gate-breaking codex nit (ZERO-flake comment too absolute since providers use PeerId::random) fixed in 17ff413 (comment-only, honest ~4e-19). TASK-214 DONE.
<!-- SECTION:NOTES:END -->
