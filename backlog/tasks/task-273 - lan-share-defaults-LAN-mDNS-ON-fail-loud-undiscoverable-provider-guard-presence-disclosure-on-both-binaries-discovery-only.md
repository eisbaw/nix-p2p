...
codex re-review (v2) VERDICT_NO_GO but narrowed: PASS on #2 external-address exclusion, #4 composite disclosure, #6 attributable 3-node e2e, #7, #8 fail-closed, ordering. Remaining: #3 (CRIT honesty) overclaims survive -> nix-p2p.nix:245 still promises "genuinely zero-config same-pin LAN sharing" (fails loud with empty supply); stale comments main.rs:1489 + e2e_harness.py:2450/6874. #4 (MED) public-share mDNS remedy wrong (says drop --libp2p-no-mdns; should say add --libp2p-mdns). #6 (MED) disclosure always prints "lan-share default" even for explicit mDNS. #1 (CRIT) is PRE-EXISTING (not a 273 regression; revert removed 273 exposure) -> explicit announce-after-fetch bypasses seed_nar + false report -> TASK-278 (interim fail-closed being added now). #5 -> TASK-277 (composite over-rejects provider-addr-only public-share, fail-closed).
