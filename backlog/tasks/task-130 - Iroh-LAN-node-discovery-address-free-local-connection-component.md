...
After TASK-132 proves, TASK-133 independently reviews, and TASK-136 admits a passing global-first Iroh milestone, add explicitly enabled LAN-local NodeId/address discovery to the shared TASK-115 runtime. This later independent component discovers candidates and establishes a real Iroh connection within a local network without peer-address, claim or content-locator injection. It publishes no content inventory and contacts no DNS/pkarr, relay, Mainline, tracker or other public infrastructure. Full zero-content-injection LAN Nix substitution remains owned by TASK-100 plus TASK-116, which consume these recorded LAN candidates.
...
- [ ] #6 The task cannot start without a valid global-iroh-pass-v1 artifact from TASK-136 matching the current tree and TASK-133 verdict. Dependency and source guards reject mere predecessor completion, no_go, stale/fabricated tokens, tracker-only partial evidence, or waived review.
<!-- AC:END -->
