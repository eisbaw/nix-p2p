...
Vendor the current n0 iroh-experiments content-discovery tracker/client at an exact upstream commit and port it from its Iroh release-candidate versions to this repository pinned Iroh/iroh-blobs versions. Retain its announce verification, but remove or compile-time-disable the experiments Mainline address-lookup integration until TASK-96/TASK-131. The tracker has no public/default deployment and must be explicitly operated/configured; it is one hint mechanism behind TASK-100, not an authority or sole path. A lying tracker can waste a dial but cannot bypass Nix signature/NarHash verification. Global content-DHT grounding is TASK-96/TASK-126/TASK-103, and dead tracker resolution is typed UNAVAILABLE rather than a clean MISS.
...
- [ ] #8 The vendored build removes or compile-time-disables its iroh-mainline-address-lookup integration and has no Mainline dependency before TASK-96/TASK-131. Its no-address proof uses TASK-89 explicit DNS/pkarr/relay resolution; a dependency/source guard fails if vendoring silently reintroduces Mainline.
<!-- AC:END -->
...
Graph correction 2026-08-11: the description historical TASK-73 global-DHT reference is superseded by TASK-96 -> TASK-126 -> TASK-103. Vendoring may retain tracker/client code only; its former iroh-mainline-address-lookup integration is removed or disabled until TASK-131.
<!-- SECTION:NOTES:END -->
