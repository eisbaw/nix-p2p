...
PREMISE-CHECK DONE (2026-08-17). Confirmed: (1) the authenticated remote PeerId IS available at the serve accept loop (fabric-libp2p/src/swarm.rs run_accept_loop: `while let Some((peer, stream)) = incoming.next()`), so a per-peer budget is keyable there. (2) ServeBudget (peer-fabric/src/budget.rs:118) is PER-SERVE (deadline + bytes-served), NOT a per-peer-aggregate derivation budget over a window. (3) 229 DeriveBudget is NOT applied on the shipped serve/regenerate path (server.rs serve(ServeBudget) -> nar.rs serve_stream admits against ServeBudget only; the CatalogNarSupplier store-dump is bounded per-serve, not per-peer-aggregate). So the gap is REAL: a hostile peer can trigger many/large regenerate-on-demand nix-store --dump across serves, per-peer-unbounded. PRIORITY NUANCE: the accept-loop doc calls per-peer serve backpressure "future hardening, not needed for the CURRENT trust model" (single-host/loopback/containers) — so 243 is PILOT-READINESS hardening (a public-share node exposed to hostile peers), not a current-trust-model integrity hole (integrity guarantee = never a bad store path, unaffected). Re-scoped work when picked up: charge DeriveBudget on serve_stream keyed by the accept-loop peer + a global ceiling, over-the-wire mutation bite. Deprioritise vs unambiguous insurance (112) until public-share pilot-readiness is the active goal or the owner steers.
<!-- SECTION:NOTES:END -->
