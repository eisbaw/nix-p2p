...
/// behind the seam. Retiring this bridge (moving the fetch path onto `NarTransfer`)
/// is the follow-up that lets `transport_iroh` move to `fabric-iroh` - TASK-149.
#[async_trait]
...
/// trait until the daemon fetch path itself adopts `NarTransfer` (TASK-149).
